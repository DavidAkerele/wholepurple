...
export default function HomeHero({ title, subtitle }: HomeHeroProps) {
  ...
  return (
    <section className="relative w-full h-[calc(100vh-88px)] lg:h-[calc(100vh-88px)] min-h-[550px] lg:min-h-[800px] bg-[#FAF7F2] flex items-end lg:items-center overflow-hidden">
      ...
      <div className="hidden lg:block absolute top-0 right-0 w-[1000px] h-[1000px] bg-purple-100/30 rounded-full blur-[120px] -translate-y-1/2 translate-x-1/4 animate-pulse z-[1]"></div>
      <div className="hidden lg:block absolute bottom-0 left-0 w-[800px] h-[800px] bg-green-50/40 rounded-full blur-[100px] translate-y-1/4 -translate-x-1/4 z-[1]"></div>

      <div className="container mx-auto px-4 md:px-8 relative z-40 pb-20 lg:pb-0">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-20 items-center max-w-[1700px] mx-auto">
          
          {/* Content Area */}
          <div className={`lg:col-span-7 flex flex-col items-start text-left transition-all duration-1000 ${mounted ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="hidden lg:inline-flex items-center gap-3 px-5 py-2.5 bg-white rounded-full border border-gray-100 shadow-sm mb-10 group cursor-default">
            ...
            <div className="flex flex-row gap-3 lg:gap-6 w-full sm:w-auto">
              <Link 
                href="/shop" 
                className="group relative flex-1 sm:flex-none px-6 lg:px-12 py-4 lg:py-6 bg-gray-900 text-white font-black text-[8px] lg:text-[10px] uppercase tracking-[0.2em] lg:tracking-[0.25em] rounded-xl lg:rounded-2xl overflow-hidden shadow-2xl shadow-black/10 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 lg:gap-4"
              >
                ...
                <ArrowRight className="relative z-10 w-4 h-4 lg:w-5 lg:h-5 group-hover:translate-x-2 transition-transform" />
              </Link>
              
              <Link 
                href="/shop/builders" 
                className="flex-1 sm:flex-none px-6 lg:px-12 py-4 lg:py-6 bg-white/60 lg:bg-white backdrop-blur-md lg:backdrop-blur-none text-gray-900 font-black text-[8px] lg:text-[10px] uppercase tracking-[0.2em] lg:tracking-[0.25em] rounded-xl lg:rounded-2xl border-2 border-gray-100/50 lg:border-gray-100 hover:border-[var(--primary-purple)] hover:text-[var(--primary-purple)] transition-all flex items-center justify-center gap-2 lg:gap-4 shadow-xl shadow-black/5"
              >
                <span className="whitespace-nowrap">Custom Boxes</span>
              </Link>
            </div>
          </div>

          {/* Right Visual Area (Desktop Only) */}
          <div className={`hidden lg:block lg:col-span-5 relative transition-all duration-1000 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="relative aspect-[4/5] rounded-[80px] overflow-hidden shadow-[0_60px_100px_-20px_rgba(99,57,145,0.2)] border-[16px] border-white bg-white group">
              {images.map((src, idx) => (
                <Image 
                  key={src}
                  src={src} 
