...
export default function CreateOrderForm({ products }: { products: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      router.push("/dashboard/shop-manager/orders");
    } else {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
      ...
      <div className="xl:col-span-2 flex flex-col gap-6">
        <div className="bg-white p-6 rounded-[32px] border border-gray-100 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {filteredProducts.map(product => (
              ...
              >
                ...
                <div>
                  <span className="text-[10px] font-black text-[var(--accent-green)] uppercase tracking-wider">{product.category.name}</span>
                  <h4 className="font-bold text-gray-900 line-clamp-1">{product.name}</h4>
                  <p className="text-[var(--primary-purple)] font-black">₦{product.price.toLocaleString()}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="flex flex-col gap-8">
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-[32px] border border-gray-100 flex flex-col gap-8 h-fit sticky top-8">
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
              <User className="w-5 h-5 text-[var(--primary-purple)]" />
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-6 flex items-center gap-2">
              <CreditCard className="w-5 h-5 text-[var(--primary-purple)]" />
              Order Summary
            </h3>
              ...
              {cart.map(item => (
                <div key={item.product.id} className="flex items-center gap-3 p-3 rounded-2xl border border-gray-50 bg-gray-50 text-gray-900/50">
                  ...
                  <div className="flex-1 min-w-0">
                    <h5 className="text-sm font-bold text-gray-900 truncate">{item.product.name}</h5>
            ...
            <div className="pt-6 border-t border-gray-100 flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between pt-3 mt-3 border-t border-dashed border-gray-200">
                ...
                <span className="text-2xl font-black text-[var(--primary-purple)]">₦{total.toLocaleString()}</span>
              </div>
            </div>
          </div>
